...
<head>
    <meta charset="utf-8">
    <title>Landlord Confirmation</title>
    <style>
        body {
...
<body>
    <h1>LANDLORD CONFIRMATION<br>(Wohnungsgeberbescheinigung)</h1>
    ...
    <div class="section">
        ...
        is registered at the following address:</p>
    </div>
    
    <div class="section">
        <p><strong>Address:</strong> {{ $booking->room->property->address }}, {{ $booking->room->property->city }}<br>
        <strong>Room/Apartment:</strong> {{ $booking->room->name }}</p>
    </div>
    
    <div class="section">
        <p><strong>Registration Period:</strong><br>
        From: {{ \Carbon\Carbon::parse($booking->start_at)->format('d.m.Y') }}<br>
        To: {{ \Carbon\Carbon::parse($booking->end_at)->format('d.m.Y') }}</p>
    </div>
